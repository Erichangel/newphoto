...
def api_music_settings():
    ...
    if request.method == 'GET':
        if user_id:
            # 读取用户专属音乐设置
            settings_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'user_{user_id}_music_settings.json')
            if os.path.exists(settings_file):
                with open(settings_file, 'r', encoding='utf-8') as f:
                    settings = json.load(f)
            else:
                # 默认设置
                settings = {'auto_switch': True, 'auto_play': True}
        else:
            # 未登录，使用全局设置
            config = load_music_config()
            settings = config.get('settings', {'auto_switch': True, 'auto_play': True})
